 ...
 * for bundler environments (Vite, Rollup, SvelteKit, Next.js). It fetches the
 * .wasm as a static URL and loads wasm_exec.js as a script so the bundler can
 * process both.
 *
...
export async function load(opts = {}) {
  ...
  const go = new Go();
  // cache: 'no-cache' — the 30 MB WASM is aggressively cached by browsers;
  // this ensures the latest binary is always loaded during development.
